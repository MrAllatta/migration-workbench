...
`make chassis-gate` is the authoritative gate. PRs must pass it. Session rituals are documented in `.pi/OPERATORS_CONTRACT.md`.
...
| Path | Purpose |
|------|---------|
| `.pi/extensions/session-harness/` | Pi chain loop (Node.js) |
| `.opencode/extensions/session-harness/` | Opencode chain loop (Python) |
| `specs/state.yaml` | Active epic/story cursor |
| `specs/execution-status.yaml` | Story completion tracking |
| `specs/epics/eNN-slug/epic.yaml` | Epic/story definitions with tasks and verify commands |
| `specs/missions/<slug>/brief.md` | Human-authored mission goals |
| `specs/missions/<slug>/journal.md` | Execution log |

### Shared Contract

Both harnesses read the same YAML files:

1. `specs/state.yaml` — which epic/story is active
2. `specs/execution-status.yaml` — which stories are complete (cursor)
3. `specs/epics/*/epic.yaml` — story definitions with tasks and verify commands

### Harness Lifecycle

1. Harness reads `state.yaml` to find active epic
2. Harness reads `epic.yaml` to get stories
3. Harness reads `execution-status.yaml` for cursor position
4. Harness formats prompt and dispatches story to agent
5. Agent executes story, updates `execution-status.yaml`
6. Harness reads cursor, advances to next story
