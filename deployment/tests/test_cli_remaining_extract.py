...
def test_deploy_parser_registers_subcommand() -> None:
    ...
    parsed = parser.parse_args(
        ["deploy", "test-space", "--env", "production", "--dry-run"]
    )
...
def test_deploy_dry_run_returns_int() -> None:
    ...
    result = m._deploy_dry_run(
        argparse.Namespace(
            space="nonexistent-space",
            env="production",
            dry_run=True,
            live=False,
            local=False,
...
def test_drift_parser_registers_check() -> None:
    ...
    parsed = parser.parse_args(
        ["drift", "check", "--baseline", "/dev/null", "--new", "/dev/null"]
    )
    assert hasattr(parsed, "func") and callable(parsed.func)
