...
def test_wb_vertical_show_example_shows_details():
    ...
    assert "Example vertical template for testing" in output, (
...
def test_wb_vertical_show_example_json_returns_valid_json():
    ...
    assert vertical.get("name") == "example", f"Expected name 'example', got: {vertical.get('name')}"
    assert vertical.get("version") == "0.1.0", f"Expected version '0.1.0', got: {vertical.get('version')}"
    assert "example" in vertical.get("description", "").lower(), f"Expected description to contain 'example', got: {vertical.get('description')}"
    ...
    assert "Widget" in entity_templates, f"Expected Widget in entity_templates, got: {list(entity_templates.keys())}"
    assert "Category" in entity_templates, f"Expected Category in entity_templates, got: {list(entity_templates.keys())}"