...
def check_venv():
    if not VENV_DIR.exists():
        print(f"FAIL[PREFLIGHT_VENV_MISSING]: {FAIL_MESSAGES['PREFLIGHT_VENV_MISSING']}")
        print("  → Action: Run `make install` or create .venv with `python -m venv .venv`")
        sys.exit(1)
    return True
...
def check_wb():
    if shutil.which("wb") is None and not WB_PATH.exists():
        print(f"FAIL[PREFLIGHT_WB_NOT_FOUND]: {FAIL_MESSAGES['PREFLIGHT_WB_NOT_FOUND']}")
        print("  → Action: Ensure wb CLI is installed and on PATH, or run `make install`")
        sys.exit(1)
    return True
...
def check_domain_context():
    ...
    if not data.get("domain"):
        print(f"FAIL[PREFLIGHT_DOMAIN_EMPTY]: {FAIL_MESSAGES['PREFLIGHT_DOMAIN_EMPTY']}")
        print("  → Action: Add non-empty 'domain' field to config/domain_context.yaml")
        sys.exit(1)

    if not data.get("year_scope", {}).get("active"):
        print(f"FAIL[PREFLIGHT_YEAR_SCOPE_EMPTY]: {FAIL_MESSAGES['PREFLIGHT_YEAR_SCOPE_EMPTY']}")
        print("  → Action: Add non-empty 'year_scope.active' list to config/domain_context.yaml")
        sys.exit(1)
    ...
    if not operational and not reference:
        print(f"FAIL[PREFLIGHT_VOCABULARY_EMPTY]: {FAIL_MESSAGES['PREFLIGHT_VOCABULARY_EMPTY']}")
        print("  → Action: Add at least one token to 'vocabulary.operational' or 'vocabulary.reference'")
        sys.exit(1)
