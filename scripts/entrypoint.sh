...
GUNICORN_EXEC="$(printf '%s migration_workbench.wsgi:application --bind 0.0.0.0:8080 --workers %s --threads %s --worker-class gthread --access-logfile - --error-logfile - --capture-output' \
